...
void main() {
    ...
    if (fp == NULL) {
        ...
        if (!fgets(fileread, sizeof(fileread), fp)) {
            // When file pointer reaches the end of the file, file transfer "completed" message is sent to the accepted client connection using newsd, socket file descriptor.
